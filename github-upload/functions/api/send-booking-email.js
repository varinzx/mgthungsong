...
async function sendBookingEmail(env, booking) {
  ...
  if (!apiKey || !to || !from) {
    return {
      ...
      body: {
        ok: false,
        message: 'ยังไม่ได้ตั้งค่า RESEND_API_KEY, BOOKING_NOTIFY_EMAIL หรือ BOOKING_FROM_EMAIL บน Cloudflare Pages'
      }
    };
  }
...
export async function onRequestPost(context) {
  try {
    ...
    const result = await sendBookingEmail(context.env, payload.booking || payload);
    return json(result.status, result.body);
  } catch (error) {
    return json(400, { ok: false, message: error.message || 'ส่งอีเมลไม่สำเร็จ' });
  }
}
